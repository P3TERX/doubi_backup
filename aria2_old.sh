...
Service_aria2(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/P3TERX/doubi_backup/master/other/aria2_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/P3TERX/doubi_backup/master/other/aria2_debian -O /etc/init.d/aria2; then
			echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
...
Installation_dependency(){
	if [[ ${release} = "centos" ]]; then
		...
		wget --no-check-certificate -O "/etc/apt/sources.list" "https://raw.githubusercontent.com/P3TERX/doubi_backup/master/sources/us.sources_stretch.list"
		apt-get update
...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/P3TERX/doubi_backup/master/aria2.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
